...
import { Bebida, TipoBebida, Pedido, BordaTipo } from "../interfaces/pedido.interface";
...
export class OrderService {
    ...
    private formatarBebidas(): string {
        ...
        return this.orderData.bebida.map(bebida => {
            const nomeFormatado = this.formatarNomeBebida(bebida.tipo);
            const quantidade = bebida.quantidade && bebida.quantidade > 1 ? ` (${bebida.quantidade}x)` : '';
            return `${nomeFormatado}${quantidade}`;
        }).join(', ');
    }

    private formatarNomeBebida(tipo: TipoBebida): string {
